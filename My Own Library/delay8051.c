...
// do not pass paramters
void delay_us (long delay){
		int a;
		for ( a = 0; a < delay; a++){
		// read the timer overflow flag // if it counts up then exit while loop
		TH0 = 0xFF; // for 1 ms second 0x00; // start from 0
		TL0 = 0xFF; // for 1 ms second //0x00; // start from 0
		TR0 = 1 ; // start the timer
		while (TF0==0);
		TR0 = 0 ; // stop the timer the timer
		TF0 = 0; // reset the overflow flag
...
void delay_10us (){
		...
		// read the timer overflow flag // if it counts up then exit while loop
		TH0 = 0xFF; // for 10 us second 0x00; // start from 0
		TL0 = 0xF6; // for 10 us second //0x00; // start from 0
		TR0 = 1 ; // start the timer
		while (TF0==0);
		TR0 = 0 ; // stop the timer the timer
		TF0 = 0; // reset the overflow flag
